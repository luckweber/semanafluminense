...
function wpcf7_add_shortcode_text() {
	wpcf7_add_shortcode(
		array( 'text', 'input', 'text*', 'email', 'email*', 'url', 'url*', 'tel', 'tel*' ),
...
function wpcf7_text_shortcode_handler( $tag ) {
	...
	if ( in_array( $tag->basetype, array( 'email', 'url', 'tel', 'input' ) ) ) {
	...
	if($tag->basetype == "input"){
		//$num = 1;
		$field = $tag->name;
		global $wpdb;
		$customers = $wpdb->get_results("SELECT * FROM wp_cf7dbplugin_submits where field_name = '$field' order by submit_time desc limit 0,1");
		$num = $customers[0]->field_value;
	...
	if($tag->basetype == "input"){
		$html = sprintf(
		'<span class="wpcf7-form-control-wrap %1$s"><input %2$s readonly/>%3$s</span>',
...
add_filter( 'wpcf7_validate_tel*', 'wpcf7_text_validation_filter', 10, 2 );
add_filter( 'wpcf7_validate_input', 'wpcf7_text_validation_filter', 10, 2 );
...
function wpcf7_add_tag_generator_text() {
	...
	$tag_generator->add( 'tel', __( 'tel', 'contact-form-7' ),
		'wpcf7_tag_generator_text' );
	$tag_generator->add( 'input', __( 'input', 'contact-form-7' ),
		'wpcf7_tag_generator_text' );	
}
...
function wpcf7_tag_generator_text( $contact_form, $args = '' ) {
	...
	if ( ! in_array( $type, array( 'email', 'url', 'tel', 'input') ) ) {
		$type = 'text';
	}
