...
function wpcf7_add_shortcode_select() {
	wpcf7_add_shortcode( array( 'base','selector','selector*', 'select', 'select*' ),
...
function wpcf7_select_shortcode_handler( $tag ) {
	...
	$atts = wpcf7_format_atts( $atts );
        ...
        if ($tag->basetype == "base"){
            $html = sprintf(
			'<span class="wpcf7-form-control-wrap %1$s"><select class="base" id="bb" %2$s>%3$s</select>%4$s</span>',
			sanitize_html_class( $tag->name ), $atts, $html, $validation_error );
        
            
        }else if ($tag->basetype == "selector"){
            
              $html = sprintf(
		'<span id ="'.$tag->name.'" class="wpcf7-form-control-wrap %1$s"><select id="'.$tag->basetype.'" %2$s>%3$s</select>%4$s</span>',
		sanitize_html_class( $tag->name ), $atts, $html, $validation_error );
        echo '<script>
				window.onload=function(){
					base = document.getElementsByClassName("base");
					selector = document.getElementById("selector");
					tag = document.getElementById("'.$tag->name.'");
					tags =  document.getElementById("wpcf7-form-control-wrap cidade");
					...
					clone = base[0].cloneNode(true);
					tags[1].insertBefore(clone, tags[1].childNodes[1]);
					...
					selector.onchange=function(){
						for(var i = 0; i <base.length ; i++){
							if(i == selector.selectedIndex ){
								clone.remove();
								clone = base[i].cloneNode(true);
								tag.insertBefore(clone, tag.childNodes[1]);
								clone.style.display = "block";
							}
						}
...
/* Validation filter */
add_filter( 'wpcf7_validate_selector', 'wpcf7_select_validation_filter', 10, 2 );
...
function wpcf7_add_tag_generator_menu() {
	...
	$tag_generator->add( 'menu', __( 'drop-down menu', 'contact-form-7' ),
        ...
        $tag_generator->add( 'selector', __( 'drop-down selector', 'contact-form-7' ),
		'wpcf7_tag_generator_menu' );
}
