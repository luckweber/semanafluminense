		...
		if($name == $acceptNumber) {
			...
			if($acceptNum != '') {
				if(ctype_digit($acceptNum)) {
					...
					$result['valid'] = false;
					$result['reason'][$name] = 'Por favor, digite apenas números.';
				}
			}
		}
	}
//__________________________________________________________________________________________________		
	...
	//acceptMoney
	$acceptMoneys = array('acceptMoney', 'acceptMoney1', 'acceptMoney2', 'acceptMoney3', 'acceptMoney4', 'acceptMoney5', 'acceptMoney6');
	
	foreach($acceptMoneys as $acceptMoney){
		if($name == $acceptMoney) {
			$acceptMon = isset( $_POST[$acceptMoney] ) ? $_POST[$acceptMoney] : '';
			
			if($acceptMon != '') {
				if( preg_match('/^[0-9]+(?:\,[0-9]+)?$/', $acceptMon ) ) {
					$result['valid'] = true;
				} else {
					$result['valid'] = false;
					$result['reason'][$name] = 'Por favor, digite valores no formato 999,99.';
				}
			}
		}
	}
...
function cf7_custom_date_validation($result, $tag) {
	$type = $tag['type'];
	$name = $tag['name'];
...
//__________________________________________________________________________________________________
	...
	$date1 = $_POST['alt-event-start-date'];
	$date2 = $_POST['alt-event-end-date'];
	
	// Start Date
	if($name == 'alt-event-start-date'){
		if(!compare_date($date1, $date2)) {
			$result['valid'] = false;
			$result['reason'][$name] = '<strong>Data Inicial</strong> não pode ser <strong>maior</strong> que a <strong>Data Final</strong>';
		}
	}
...
//__________________________________________________________________________________________________
	...
	// End Date
	if($name == 'alt-event-end-date'){
		if(!compare_date($date1, $date2)) {
			$result['valid'] = false;
			$result['reason'][$name] = '<strong>Data Final</strong> não pode ser <strong>menor</strong> que a <strong>Data Inicial</strong>';
		}
	}
...
//add fiter for text area validation
add_filter( 'wpcf7_validate_date', 'cf7_custom_date_validation', 10, 2 ); // Date Field
add_filter( 'wpcf7_validate_date*', 'cf7_custom_date_validation', 10, 2 ); // Required Date Field

/*
TIME VALIDATIONS
*/

function cf7_custom_time_validation($result, $tag) {
	$type = $tag['type'];
	$name = $tag['name'];
	
	//if empty give required field error
	if($type == 'select*' && $_POST[$name] == ''){
			$result['valid'] = false;
			$result['reason'][$name] = wpcf7_get_message( 'invalid_required' );
	}
...
//__________________________________________________________________________________________________
	...
	$time1 = $_POST['event-start-time-h'] . ':' . $_POST['event-start-time-m'];
	$time2 = $_POST['event-end-time-h'] . ':' . $_POST['event-end-time-m'];
	
	// Start Date
	if($name == 'event-start-time-h'){
		if(!compare_time($time1, $time2)) {
			$result['valid'] = false;
			$result['reason'][$name] = '<strong>Hora Inicial</strong> não pode ser <strong>maior</strong> que a <strong>Hora Final</strong>';
		}
	}
...
//__________________________________________________________________________________________________
	...
	// End Date
	if($name == 'event-end-time-h'){
		if(!compare_time($time1, $time2)) {
			$result['valid'] = false;
			$result['reason'][$name] = '<strong>Hora Final</strong> não pode ser <strong>menor</strong> que a <strong>Hora Inicial</strong>';
		}
	}
...
//add fiter for text area validation
//add_filter( 'wpcf7_validate_select', 'cf7_custom_time_validation', 10, 2 ); // Time Field
//add_filter( 'wpcf7_validate_select*', 'cf7_custom_time_validation', 10, 2 ); // Required Time Field
